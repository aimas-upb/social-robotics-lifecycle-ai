...
class ExploreFindPerson(py_trees.behaviour.Behaviour):
    def __init__(self, node: Node, person="Master", name="FindPerson"):
        super().__init__(name)
        self.node = node
        self.person = person
...
class Condition_FaceRecognized(py_trees.behaviour.Behaviour):
    def __init__(self, node: Node, person="Master", name="Condition_FacialRecognition"):
        super().__init__(name)
        self.node = node
        self.person = person
        ...
        if Config.current_room in Config.people_rooms:
            ...
            # We could update the memory for the person we found, but we don't know who they are with the current tech
                ...
                if self.person in Config.people_room_memory:
                    ...
                    actual_person = Config.people_rooms[Config.current_room]
                    Config.people_room_memory[actual_person] = Config.current_room
